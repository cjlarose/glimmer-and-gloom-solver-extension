...
function clearOverlay() {
  const existingOverlay =
    document.querySelector<HTMLElement>(".solution-overlay");
  if (existingOverlay) {
...
function applySolutionOverlay(
  ...
  const solutionSet = new Set(
    solution.map(({ row, column }) => `${row},${column}`),
  );
...
function handleConnectionState(connectionState: ConnectionState) {
  ...
  switch (connectionState.type) {
    case "COMPUTED_SOLUTION": {
      const { difficulty, level, minimalSolution, clickedCoords } =
        connectionState;
